...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "@OFFICIALTEAMERROR"
...
async def amireallyalive(alive):
    ...
    if ALV_PIC:
        tele = f"**Welcome To VENOM'S BOT**\n\n"
        tele += f"`{CUSTOM_ALIVE}`\n\n"
        tele += (
            f"{telemoji} **Telethon version**: `1.17`\n{telemoji} **Python**: `3.8.3`\n"
        )
        tele += f"{telemoji} **VENOM Bot Version**: `{telever}`\n"
        tele += f"{telemoji} **More Info**: @OFFICIALTEERROR\n"
        ...
        tele += "    [✨ GitHub Repository ✨](https://github.com/NICK-FTW/TeleBot)"
    ...
    with BytesIO() as sticker:
        ...
        await borg.send_message(
            ...
            f"`{CUSTOM_ALIVE}`\n\n"
            f"{telemoji} **Telethon version**: `1.17`\n{telemoji} **Python**: `3.8.3`\n"
            f"{telemoji} **VENOM Bot Version**: `{telever}`\n"
            f"{telemoji} **More Info**: @OFFICIALTEAMERROR\n"
            f"{telemoji} **Sudo** : `{sudo}`\n"
            f"{telemoji} **TeleBot Uptime**: `{uptime}`\n"
            f"{telemoji} **Database Status**: `All OK 👌!`\n"
            f"{telemoji} **My pro owner** : [{DEFAULTUSER}](tg://user?id={myid})\n\n"
            "    [✨ GitHub Repository ✨](https://github.com/NICK-FTW/TeleBot)",
            link_preview=False,
        )
